...
public class BoardDAO {
	...
	public List<BoardVO> selectList() {
		// order by seq desc : seq에 desc(역순)으로 정렬
	...
	public BoardVO selectOne(BoardVO vo) {
		// content는 중복될수도있어서 seq로 글을찾아야함
		sql = "select * from board where seq=?";
		try {
			pst = cn.prepareStatement(sql);
			pst.setInt(1, vo.getSeq());// vo에서 seq값을 꺼내서 첫번째 ?에 넣어준다
			rs = pst.executeQuery(); //pst에 한걸 rs에 담아줘야함
			// => 결과확인
			if (rs.next()) {
				vo.setSeq(rs.getInt(1));
				vo.setId(rs.getString(2));
	...
	public int delete(BoardVO vo) {
		...
		if(vo.getSeq()==vo.getRoot()) {
			...
			//		  where 의 비교 컬럼만 root이고, ? 는 seq로 가능
