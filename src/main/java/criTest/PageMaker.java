...
public class PageMaker {
	private int totalRowsCount; // 전체Row 갯수 (전체 Page 수 계산 위해 필요)
	...
	private boolean next; // 이전 PageBlock 으로
	...
	public void calcData() {
		// 3.1) currPage가 속한 페이지블럭의 epageNo를 계산
	...
	//	  최종적으로 QueryString을 return해서 type을 string으로 함
	public String makeQuery(int currPage) {
		// UriComponents는 new를 이용해 생성하지 않고 UriComponentsBuilder를 이용해 생성함
		...
		//uriComponents가 string이 아니라.toString()으로 string로 바꿔줌
		
	} //makeQuery
	
	// ** ver02
	// => uri 에 search 기능 추가
	// searchType이랑  keyword가 붙어 다녀야 Paging 시에도 조건이 유지되도록 해줘야함
